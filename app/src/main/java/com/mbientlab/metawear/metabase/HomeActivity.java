...
public class HomeActivity extends AppCompatActivity implements ActivityBus, ServiceConnection, ScannerFragment.EventBus, DeviceInfoFragment.MetaCloudOptions {
    ...
    /**
     * The Show tutorial string
     */
    static final String SHOW_TUTORIAL = "show_tutorial";

    /**
     * Teardown and disconnect task.
     *
     ...
     * @param retries    the maximum retry number
     * @param showDialog the show dialog boolean, determine if show the dialog
     * @return the task
     */
    ...
    public void onBackPressed() {
        if (backstack.isEmpty()) {
            scanner.stop();
            if(!DeviceInfoFragment.inNewSession) {
                super.onBackPressed();
            }
        } else {
